...
8. 如果是 `bash` 且 `wrapWithSrt = true`，从 effective permission profile 生成 SRT settings 并改写命令。
...
[core/srt.ts](../core/srt.ts) 会把当前 effective permission profile 转换为 SRT settings，写入 `~/.pi/agent/extensions/pi-perm/runtime/<tool-call-id>.srt-settings.json`，再将原始命令改写为：
...
| `bash` 命令 | workspace 边界内普通命令自动放行；危险操作、禁网联网命令可确认/阻断，也可被 SRT 包装 |
